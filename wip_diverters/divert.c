// doas pkg_add -v protobuf-c py3-protobuf // doesn't work yet
#include <stdlib.h>
#include <sys/types.h>
...
int main(int argc, char *argv[])
{
	...
	/*** BEGIN PROTOBUF STUFF THAT DOESN'T WORK YET ********/
  ...
  free(msg_buf);
	*/
	/****** END PROTOBUF STUFF THAT DOESN'T WORK YET *****/
  ...
  // TODO: make another sockaddr_in on the port for our app and sendto() there. Then recvfrom() back. Will also need another sockaddr_in for the second recvfrom()?
	...
	for (;;) {
		...
		// TODO: Figure out why this sometimes combines/repeats the same data in a single send()
		// 				- Does it buffer until the python socket sends data back? It has a lot of buffers when I added the DNS lookup. Didn't see that before or after...
		snprintf(buf, sizeof(buf), "{\"src_host\": \"%s\", \"src_port\": %u, \"dst_host\": \"%s\", \"dst_port\": %u}", src, ntohs(th->th_sport), dst, ntohs(th->th_dport));
    //write(client_fd, buf, strlen(buf));
		...
		PrintData(packet, hlen);

		printf("TCP Header\n");
		PrintData(packet + hlen, th->th_off*4);
		
		/* printf("UDP Header\n"); */
		/* PrintData(packet + iphdrlen , sizeof icmph); */

		printf("Data Payload\n");
		//PrintData(packet + hlen /*+ sizeof icmph */, (n /*- sizeof icmph*/ - hlen));
		// TODO: Need to check for endianness?
		PrintData(packet + hlen + th->th_off*4, (n - (th->th_off*4) - hlen));
